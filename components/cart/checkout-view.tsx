...
type PaymentErrors = Partial<Record<"mobileNumber" | "cardNumber" | "expiration" | "cvc", string>>;
...
function OrderSummary({ locale, labels, items, subtotal, shipping, total, hidden }: { locale: Locale; labels: Labels; items: CartItem[]; subtotal: number; shipping: number; total: number; hidden: boolean }) {
  ...
  return <aside className="checkout-order-summary"><h2>{labels.yourOrder}</h2>{items.map((item) => <div className="checkout-summary-item" key={item.id}><Image src={item.image} alt={item.name} width={44} height={44} /><div><strong>{item.name}</strong><span>{labels.size} {item.size} Â· {item.color}</span></div><b>{formatPrice(item.price * item.quantity, locale)}</b></div>)}<div className="checkout-total-lines"><div><span>{labels.subtotal}</span><span>{formatPrice(subtotal, locale)}</span></div><div><span>{labels.shipping}</span><span>{formatPrice(shipping, locale)}</span></div><div><strong>{labels.total}</strong><strong>{formatPrice(total, locale)}</strong></div></div></aside>;
}
  ...
  return (
    <main className="checkout-page">
      ...
      <div className="checkout-body"><div className="checkout-grid"><div className="checkout-main">
        {step === 1 && <section className="checkout-step-content"><div className="checkout-guest">{labels.guest}<Link href={`/${locale}/compte/connexion`}>{labels.login}</Link></div><h1>{labels.delivery}</h1><CheckoutField id="fullName" label={labels.fullName} value={deliveryDetails.fullName} placeholder={labels.fullNamePlaceholder} error={deliveryErrors.fullName} onChange={(value) => updateDelivery("fullName", value)} /><div className="checkout-field-row"><CheckoutField id="phone" label={labels.phone} value={deliveryDetails.phone} placeholder={labels.phonePlaceholder} error={deliveryErrors.phone} onChange={(value) => updateDelivery("phone", value)} /><CheckoutField id="email" label={labels.email} value={deliveryDetails.email} placeholder={labels.emailPlaceholder} error={deliveryErrors.email} onChange={(value) => updateDelivery("email", value)} /></div><div className="checkout-field"><label htmlFor="country">{labels.country}</label><select id="country" value={deliveryDetails.country} onChange={(event) => updateDelivery("country", event.target.value)}><option>Cameroun</option><option>CÃ´te d&apos;Ivoire</option><option>France</option><option>Autre</option></select></div><div className="checkout-field-row"><CheckoutField id="city" label={labels.city} value={deliveryDetails.city} placeholder={labels.cityPlaceholder} error={deliveryErrors.city} onChange={(value) => updateDelivery("city", value)} /><CheckoutField id="neighborhood" label={labels.neighborhood} value={deliveryDetails.neighborhood} placeholder={labels.neighborhoodPlaceholder} error={deliveryErrors.neighborhood} onChange={(value) => updateDelivery("neighborhood", value)} /></div><CheckoutField id="address" label={labels.address} value={deliveryDetails.address} placeholder={labels.addressPlaceholder} error={deliveryErrors.address} onChange={(value) => updateDelivery("address", value)} /><div className="checkout-delivery-card"><strong>ðŸšš {labels.estimated}</strong><span>{labels.estimatedDetail}</span></div><button className="checkout-primary" onClick={() => validateDelivery() && goTo(2)}>{labels.continuePayment}</button></section>}

        {step === 2 && <section className="checkout-step-content"><h1>{labels.payment}</h1><PaymentOption selected={payment === "mm"} onSelect={() => { setPayment("mm"); setPaymentErrors({}); }} title={labels.mobileMoney} note={labels.mobileMoneyNote}><CheckoutField id="mobileNumber" label={labels.mobileNumber} value={mobileNumber} placeholder={labels.phonePlaceholder} error={paymentErrors.mobileNumber} onChange={(value) => { setMobileNumber(value); setPaymentErrors((current) => { const remaining = { ...current }; delete remaining.mobileNumber; return remaining; }); }} /></PaymentOption><PaymentOption selected={payment === "card"} onSelect={() => { setPayment("card"); setPaymentErrors({}); }} title={labels.card}><CheckoutField id="cardNumber" label={labels.cardNumber} value={cardDetails.number} placeholder="0000 0000 0000 0000" error={paymentErrors.cardNumber} onChange={(value) => { updateCard("number", value); setPaymentErrors((current) => { const remaining = { ...current }; delete remaining.cardNumber; return remaining; }); }} /><div className="checkout-field-row"><CheckoutField id="expiration" label={labels.expiration} value={cardDetails.expiration} placeholder="MM/AA" error={paymentErrors.expiration} onChange={(value) => { updateCard("expiration", value); setPaymentErrors((current) => { const remaining = { ...current }; delete remaining.expiration; return remaining; }); }} /><CheckoutField id="cvc" label={labels.cvc} value={cardDetails.cvc} placeholder="123" error={paymentErrors.cvc} onChange={(value) => { updateCard("cvc", value); setPaymentErrors((current) => { const remaining = { ...current }; delete remaining.cvc; return remaining; }); }} /></div></PaymentOption><PaymentOption selected={payment === "cod"} onSelect={() => { setPayment("cod"); setPaymentErrors({}); }} title={labels.cod} note={labels.codNote} /><button className="checkout-primary checkout-button-spaced" onClick={() => validatePayment() && goTo(3)}>{labels.continueSummary}</button><button className="checkout-link-button" onClick={() => goTo(1)}>â† {labels.editDelivery}</button></section>}

        {step === 3 && <section className="checkout-step-content"><h1>{labels.summary}</h1><SummaryBlock title={labels.deliveryAddress} onEdit={() => goTo(1)} edit={labels.edit}>{deliveryDetails.fullName} â€” {deliveryDetails.neighborhood}, {deliveryDetails.city}, {deliveryDetails.country}</SummaryBlock><SummaryBlock title={labels.paymentMethod} onEdit={() => goTo(2)} edit={labels.edit}>{paymentLabel}</SummaryBlock><SummaryBlock title={`${labels.items} (${items.reduce((sum, item) => sum + item.quantity, 0)})`}>{items.length ? items.map((item) => `${item.name} Ã— ${item.quantity}`).join(", ") : "-"}</SummaryBlock><label className="checkout-check"><input type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} />{labels.accept}</label><button className="checkout-primary" disabled={!accepted} onClick={() => goTo(4)}>{labels.confirm}</button></section>}

        {step === 4 && <section className="checkout-confirmation"><div className="checkout-confirm-icon"><Check aria-hidden="true" /></div><h1>{labels.thanks}</h1><p>{labels.confirmationSent}</p><strong>{labels.order} #KS-10234</strong><div className="checkout-confirm-summary"><div><span>{labels.estimated}</span><span>24â€“48h</span></div><div><span>{labels.paymentMethod}</span><span>{paymentLabel}</span></div><div><strong>{labels.paidTotal}</strong><strong>{formatPrice(total, locale)}</strong></div></div><button className="checkout-secondary">ðŸ’¬ {labels.follow}</button><div className="checkout-account-prompt"><strong>{labels.createAccount}</strong><p>{labels.createAccountText}</p><button className="checkout-primary">{labels.create}</button></div><Link className="checkout-link-button" href={`/${locale}/boutique`}>{labels.continueShopping}</Link></section>}
